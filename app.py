...
@app.route('/events', methods=['GET', 'POST'], endpoint='events')
@jwt_required()
def handle_events():
    cursor = None
    try:
        ...
        if request.method == 'GET':  # READ all events
            cursor.execute("""
            ...
            result = [{
                ...
                'Event_Status_Description': event['Event_Status_Description'],
                'Potential_Cost': str(event['Potential_Cost'])
            } for event in events]

            return jsonify(result), 200

        elif request.method == 'POST':  # CREATE a new event
            data = request.json
            try:
                cursor.execute("""
                    INSERT INTO events (
...
@app.route('/events/<int:event_id>', methods=['GET', 'PUT', 'DELETE'], endpoint='event_operations')
@jwt_required()
